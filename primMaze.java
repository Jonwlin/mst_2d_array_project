...
public class primMaze {
  ...
  public primMaze(int xSize, int ySize, int seed) {
    ...
    replaceNumber = 1; // default, should only be changed/ use dif constructor
                       // when debugging
  }

  public primMaze(int seed, primMaze otherPrim) // lets you instantiate a new
                                                // prim
  ...
  public primMaze(int seed, primMaze otherPrim, int replaceNumber) // lets you
                                                                   // instantiate
                                                                   // a new prim
  { // and copies over the visitedPoints. new seed and bagOfPoints
    this.xSize = otherPrim.getxSize();
  ...
  public primMaze(int xSize, int ySize, int seed, int replaceNumber) {
    ...
    this.seed = seed;
    random = new Random(seed);
    this.replaceNumber = 1; // default, should only be changed/ use dif
  ...
  /*
   * this is a slow method. Maybe implement stuff later if it slows program down
   * but its only a 2d array so
   */
  private point getFragmentedPoint() {
    int xStart = randInt(0, xSize - 1);
    int yStart = randInt(0, ySize - 1);
    while (!(primArray[xStart][yStart] == 0)) // checks if starting point is a
                                              // "visited" point. I could find a
                                              // faster way to do this but it
    { // it only needs to really do it once
      xStart = randInt(0, xSize - 1); // Uhhmmm embarrassing needed to be - 1
                                      // because reasons
  ...
  public LinkedList<point> getHeaderPoint(int size) {
    ...
    outerloop: for (int x = 0; x < xSize; x++) {
  ...
  **/
  ...
  private void updateBag(int x, int y) {
    ...
    try {
      if (primArray[x][y + 1] == 0) {
        if (!headerHash.contains(new point(x, y + 1))
            && bagOfPointsHash.add(new point(x, y + 1))) // if element not in
                                                         // set..
        {
    ...
    try {
      if (primArray[x + 1][y] == 0) {
        if (!headerHash.contains(new point(x + 1, y))
            && bagOfPointsHash.add(new point(x + 1, y))) // if element not in
                                                         // set..
        {
  ...
  private void addElementToPrimArray(int x, int y, int value) // security
                                                              // measures to
                                                              // ensure a point
                                                              // shouldnt be
                                                              // added where it
