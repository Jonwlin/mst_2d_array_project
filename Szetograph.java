...
public class Szetograph {
  ...
  protected void writeInformationToNewImage(primMaze prim) {
    ...
    point currentPoint;
    /* starting point */
    picture.set(startingPoint.getX(), startingPoint.getY(),
        changeColor(picture.get(startingPoint.getX(), startingPoint.getY()),
            getAllBits().remove(0), getAllBits().remove(0),
            getAllBits().remove(0)));
    for (int a = 0; a < getAllBits().size(); a++) {
      ...
      picture.set(currentPoint.getX(), currentPoint.getY(),
          changeColor(picture.get(currentPoint.getX(), currentPoint.getY()),
              getAllBits().remove(0), getAllBits().remove(0),
              getAllBits().remove(0)));
    }
  ...
  protected void writeInformationToSpecificBit(point editedPoint, String val1,
      String val2, String val3) // if
  // you
  // want
  // to
  // write
  ...
  protected String extractInformation(primMaze prim) {
    ...
    char currentChar;
    /* starting point */
    addColorToHiddenInfo(
        picture.get(startingPoint.getX(), startingPoint.getY()));
    ...
    while (!prim.isPrimFinished()) {
      ...
      currentPoint = prim.getPointStep();
      addColorToHiddenInfo(
          picture.get(currentPoint.getX(), currentPoint.getY()));
      if (hiddenInfo.size() % 4 == 0) {
        currentChar = getCharacterFromHiddenList();
        if ((int) currentChar == 27) // if char and next char are exit
        // chars
        {
          currentChar = getCharacterFromHiddenList();
  ...
  protected void extractInformationFromLinkedList(LinkedList<point> llPoint) {
    ...
    while (!llPoint.isEmpty()) {
      addColorToHiddenInfo(
          picture.get(llPoint.get(0).getX(), llPoint.get(0).getY()));
      llPoint.remove(0);
    }
  ...
  protected LinkedList<String> generateBinaryBits(String message) {
    ...
    // *3 because we have 3 different colors. This needs to change. Cant use
    // plausible dengiability with this
    for (int a = returnedList.size(); a < (picture.height() * picture.width())
        * 3; a++) // after
    // we
    // fill
    // the
  ...
  protected void updateHoldingNextBits(char character) {
    ...
    holdingNextBits = new ArrayList<String>(
        Arrays.asList(binary.split("(?<=\\G..)"))); // splits
    // string
    // into
    // bits
    // of
  ...
  protected Color changeColor(Color changedColor, String val1, String val2,
      String val3) {
    return new Color(changeLSB(changedColor.getRed(), val1),
        changeLSB(changedColor.getGreen(), val2),
        changeLSB(changedColor.getBlue(), val3));
  }
  ...
  protected String getColorLSB(Color color) {
    assert (color != null);
    return getLSB(color.getRed(), 2) + getLSB(color.getGreen(), 2)
        + getLSB(color.getBlue(), 2);
  }
  ...
  public String getDecypheredString() {
    ...
    while (!hiddenInfo.isEmpty() && hiddenInfo.size() > 16) // if not empty
    // and current
    // value is not
    // exit value
    {
    ...
    if (!((int) returnedChar == 27)) {
      return null; // if last character wasnt a returnChar then something
      // wasnt working as intended, or plausible
    }
  ...
  public char getCharacterFromHiddenList() {
    ...
    String word = hiddenInfo.remove(0) + hiddenInfo.remove(0)
        + hiddenInfo.remove(0) + hiddenInfo.remove(0);
